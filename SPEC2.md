...
- Formal semantics: capability lattice proofs in `spec/formal/capabilities.lean` (5 of 6 theorems proved — lattice laws + `cap_sendable`/`discharge_sendable`; `linear_at_most_once` is `sorry`, documented as requiring context-splitting semantics). Core HM type soundness in `spec/formal/types.lean` is **fully proved** (2026-08-14): the whole chain — `weakening` (corrected statement: the naive head-form is false for open schemes), `progress`, `closed_type_under_closed_context`, `value_has_closed_type`, `substitution_lemma` (corrected statement: closed-value + type-closed/monomorphic-context hypotheses), `preservation`, and `type_soundness` — compiles with zero `sorry`s under `lake build`. The 2026-07-26 Lean 4.16.0 regression is fully repaired; the statements that were subtly false were corrected and the corrections documented (see `spec/formal/README.md`).
