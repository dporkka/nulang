...
- Formal semantics: capability lattice proofs in `spec/formal/capabilities.lean` (5 of 6 theorems proved — lattice laws + `cap_sendable`/`discharge_sendable`; `linear_at_most_once` is `sorry`, documented as requiring context-splitting semantics). Core HM type soundness in `spec/formal/types.lean` is **partially proved** (2026-08-14): the four soundness-support theorems `weakening` (corrected statement — the naive head-form is false for open schemes), `progress`, `closed_type_under_closed_context`, and `value_has_closed_type` are proved; `substitution_lemma`/`context_drop_shadowed`/`preservation`/`type_soundness` remain `sorry` (they were regressed 2026-07-26 by a Lean 4.16.0 compatibility fix, undocumented until 2026-08-02; the substitution lemma additionally needs term-free-variable/context-invariance machinery — its naive statement is subject to the same capture subtlety as weakening). See `spec/formal/README.md`.
