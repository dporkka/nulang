...
fn emit_extract_payload(builder: &mut FunctionBuilder, raw: Value) -> Value {
...
mod typed_tests {
    ...
    /// When one operand is typed and the other is not, the compiler
    /// falls back to runtime helpers. This test verifies correct fallback
