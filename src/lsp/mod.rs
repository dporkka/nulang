...
impl LanguageServer for NulangLanguageServer {
    async fn initialize(&self, _: InitializeParams) -> Result<InitializeResult> {
        Ok(InitializeResult {
            capabilities: ServerCapabilities {
                ...
                signature_help_provider: Some(SignatureHelpOptions { trigger_characters: Some(vec!["(".to_string(), ",".to_string()]), retrigger_characters: None, work_done_progress_options: WorkDoneProgressOptions::default() }),
                document_formatting_provider: Some(OneOf::Left(true)),
                code_action_provider: Some(CodeActionProviderCapability::Simple(true)),
                semantic_tokens_provider: Some(SemanticTokensServerCapabilities::SemanticTokensOptions(SemanticTokensOptions {
                    work_done_progress_options: WorkDoneProgressOptions::default(),
                    legend: SemanticTokensLegend {
                        token_types: vec![SemanticTokenType::KEYWORD, SemanticTokenType::FUNCTION, SemanticTokenType::VARIABLE, SemanticTokenType::TYPE, SemanticTokenType::CLASS, SemanticTokenType::STRING, SemanticTokenType::NUMBER, SemanticTokenType::OPERATOR, SemanticTokenType::COMMENT, SemanticTokenType::NAMESPACE],
                        token_modifiers: vec![SemanticTokenModifier::DECLARATION, SemanticTokenModifier::READONLY],
                    }, range: None, full: Some(SemanticTokensFullOptions::Bool(true)),
                })),
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        let source = match docs.get(&params.text_document_position_params.text_document.uri) {
            Some(doc) => doc.source.clone(), None => return Ok(None) };
        drop(docs);
    ...
    async fn references(&self, params: ReferenceParams) -> Result<Option<Vec<Location>>> {
        ...
        let source = match docs.get(&params.text_document_position.text_document.uri) {
            Some(doc) => doc.source.clone(), None => return Ok(None) };
        drop(docs);
        Ok(Some(self.find_refs(&source, params.text_document_position.position)))
    }
    ...
    async fn rename(&self, params: RenameParams) -> Result<Option<WorkspaceEdit>> {
        ...
        let edits: Vec<TextEdit> = locs.iter().map(|l| TextEdit { range: l.range, new_text: params.new_name.clone() }).collect();
    ...
    async fn formatting(&self, params: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        ...
        if formatted == source { return Ok(None); }
        let lines = source.lines().count() as u32;
        Ok(Some(vec![TextEdit { range: Range { start: Position::new(0,0), end: Position::new(lines,0) }, new_text: formatted }]))
    }
    ...
    async fn semantic_tokens_full(&self, params: SemanticTokensParams) -> Result<Option<SemanticTokensResult>> {
        ...
        let source = match docs.get(&params.text_document.uri) {
            Some(doc) => doc.source.clone(), None => return Ok(None) };
        drop(docs);
        Ok(Some(SemanticTokensResult::Tokens(SemanticTokens { result_id: None, data: self.sem_tokens(&source) })))
    }
...
impl NulangLanguageServer {
    ...
    fn hover_at(&self, source: &str, position: Position) -> Option<Hover> {
        let line = position.line as usize; let col = position.character as usize;
        ...
        let mut tc = TypeChecker::new(); let mt = tc.check_module(&ast).ok()?;
        for decl in &ast.decls {
            if let crate::ast::Decl::Function { name, params, ret_type, .. } = decl {
                if name == word {
                    let p = params.iter().map(|(n,t)| format!("{}: {}", n, t.as_ref().map(|ty| format!("{:?}",ty)).unwrap_or_else(|| "?".into()))).collect::<Vec<_>>().join(", ");
                    let r = ret_type.as_ref().map(|ty| format!("{:?}",ty)).unwrap_or_else(|| "?".into());
                    return Some(Hover { contents: HoverContents::Scalar(MarkedString::String(format!("fn {}({}) -> {}", name, p, r))), range: None });
                }
            }
            if let crate::ast::Decl::Actor { name, .. } = decl { if name == word { return Some(Hover { contents: HoverContents::Scalar(MarkedString::String(format!("actor {}", name))), range: None }); } }
        }
        let kw = ["let","fn","fun","actor","agent","if","else","match","case","for","in","spawn","send","ask","perform","handle","emit","return","unit","nil","true","false"];
        if kw.contains(&word) { return Some(Hover { contents: HoverContents::Scalar(MarkedString::String(format!("keyword `{}`", word))), range: None }); }
        Some(Hover { contents: HoverContents::Scalar(MarkedString::String(format!("module type: {:?}", mt))), range: None })
    }

    fn word_at<'a>(&self, line: &'a str, col: usize) -> Option<&'a str> {
        if col >= line.len() { return None; }
        let b = line.as_bytes();
        if !b[col].is_ascii_alphanumeric() && b[col] != b'_' { return None; }
        let mut s = col; while s > 0 && (b[s-1].is_ascii_alphanumeric() || b[s-1] == b'_') { s -= 1; }
        let mut e = col; while e < b.len() && (b[e].is_ascii_alphanumeric() || b[e] == b'_') { e += 1; }
        Some(&line[s..e])
    }
        ...
        for decl in &ast.decls { if let Some(loc) = self.find_decl(decl, word) { return Some(GotoDefinitionResponse::Scalar(loc)); } }
        None
    }
    fn find_decl(&self, decl: &crate::ast::Decl, word: &str) -> Option<Location> {
        use crate::ast::Decl;
        let loc = |s: &crate::types::Span| Location { uri: Url::parse("file:///current.nu").unwrap(), range: Range { start: Position::new(s.line.saturating_sub(1) as u32, s.column.saturating_sub(1) as u32), end: Position::new(s.line.saturating_sub(1) as u32, s.column.saturating_sub(1) as u32) }};
        match decl {
            Decl::Function { name, span, .. } | Decl::Actor { name, span, .. } | Decl::Agent { name, span, .. } | Decl::Workflow { name, span, .. } | Decl::TypeAlias { name, span, .. } if name == word => Some(loc(span)),
            Decl::Module { name, decls, span, .. } => { if name == word { return Some(loc(span)); } for d in decls { if let Some(l) = self.find_decl(d, word) { return Some(l); } } None }
            _ => None,
        }
    }
    fn find_refs(&self, source: &str, position: Position) -> Vec<Location> {
        let line = position.line as usize; let col = position.character as usize;
        let target_line = match source.lines().nth(line) { Some(l) => l, None => return vec![] };
        let word = match self.word_at(target_line, col) { Some(w) => w.to_string(), None => return vec![] };
        let tokens = match Lexer::new(source).lex() { Ok(t) => t, Err(_) => return vec![] };
        let ast = match Parser::new(tokens).parse_module() { Ok(a) => a, Err(_) => return vec![] };
        let _ = TypeChecker::new().check_module(&ast);
        let mut locs = Vec::new();
        for decl in &ast.decls { self.collect_refs(decl, &word, &mut locs); }
        locs
    }
    fn collect_refs(&self, decl: &crate::ast::Decl, word: &str, locs: &mut Vec<Location>) {
        ...
        let loc = |s: &crate::types::Span| Location { uri: Url::parse("file:///current.nu").unwrap(), range: Range { start: Position::new(s.line.saturating_sub(1) as u32, s.column.saturating_sub(1) as u32), end: Position::new(s.line.saturating_sub(1) as u32, s.column.saturating_sub(1) as u32) }};
        match decl {
            Decl::Function { name, body, span, .. } => { if name == word { locs.push(loc(span)); } self.refs_expr(body, word, locs); }
            Decl::Actor { name, behaviors, span, .. } => { if name == word { locs.push(loc(span)); } for b in behaviors { self.refs_expr(&b.body, word, locs); } }
            Decl::Agent { name, span, .. } | Decl::Workflow { name, span, .. } | Decl::TypeAlias { name, span, .. } if name == word => { locs.push(loc(span)); }
            Decl::Module { name, decls, span, .. } => { if name == word { locs.push(loc(span)); } for d in decls { self.collect_refs(d, word, locs); } }
            _ => {}
        }
    }
    fn refs_expr(&self, expr: &crate::ast::Expr, word: &str, locs: &mut Vec<Location>) {
        ...
        let loc = |s: &crate::types::Span| Location { uri: Url::parse("file:///current.nu").unwrap(), range: Range { start: Position::new(s.line.saturating_sub(1) as u32, s.column.saturating_sub(1) as u32), end: Position::new(s.line.saturating_sub(1) as u32, s.column.saturating_sub(1) as u32) }};
        match expr {
            Expr::Var(name, span) => if name == word { locs.push(loc(span)); }
            Expr::Binary { left, right, .. } => { self.refs_expr(left, word, locs); self.refs_expr(right, word, locs); }
            Expr::Let { name, value, body, span } => { if name == word { locs.push(loc(span)); } self.refs_expr(value, word, locs); self.refs_expr(body, word, locs); }
            Expr::Block { exprs, .. } => { for e in exprs { self.refs_expr(e, word, locs); } }
            Expr::App { func, args, .. } => { self.refs_expr(func, word, locs); for a in args { self.refs_expr(a, word, locs); } }
            Expr::If { cond, then_branch, else_branch, .. } => { self.refs_expr(cond, word, locs); self.refs_expr(then_branch, word, locs); if let Some(ref eb) = else_branch { self.refs_expr(eb, word, locs); } }
            Expr::Pipe { left, right, .. } => { self.refs_expr(left, word, locs); self.refs_expr(right, word, locs); }
            _ => {}
        }
    }
    ...
    fn collect_syms(&self, decl: &crate::ast::Decl, syms: &mut Vec<SymbolInformation>) {
        ...
        let si = |name: &str, kind: SymbolKind, span: &crate::types::Span| SymbolInformation {
            name: name.to_string(), kind, tags: None, deprecated: None,
            location: Location { uri: Url::parse("file:///current.nu").unwrap(), range: Range {
                start: Position::new(span.line.saturating_sub(1) as u32, span.column.saturating_sub(1) as u32),
                end: Position::new(span.line.saturating_sub(1) as u32, span.column.saturating_sub(1) as u32)
            }}, container_name: None };
        match decl {
            Decl::Function { name, span, .. } => syms.push(si(name, SymbolKind::FUNCTION, span)),
            Decl::Actor { name, behaviors, span, .. } => { syms.push(si(name, SymbolKind::CLASS, span)); for b in behaviors { syms.push(si(&format!("{}.{}", name, b.name), SymbolKind::METHOD, &b.span)); } }
            ...
            Decl::Module { name, decls, span, .. } => { syms.push(si(name, SymbolKind::NAMESPACE, span)); for d in decls { self.collect_syms(d, syms); } }
        ...
        let func_name = prefix.trim_end_matches(|c: char| c.is_whitespace() || c == '(' || c == ',')
            .rsplit(|c: char| c.is_whitespace() || c == '(' || c == ',').next()?;
        if func_name.is_empty() || func_name == "let" || func_name == "if" { return None; }
        ...
        for decl in &ast.decls {
            if let crate::ast::Decl::Function { name, params, .. } = decl {
                if name == func_name {
                    let label = format!("fn {}({})", name, params.iter().map(|(n,t)| format!("{}: {}", n, t.as_ref().map(|ty| format!("{:?}",ty)).unwrap_or_else(|| "?".into()))).collect::<Vec<_>>().join(", "));
                    return Some(SignatureHelp { signatures: vec![SignatureInformation { label, documentation: None, parameters: None, active_parameter: None }], active_signature: Some(0), active_parameter: Some(comma_count as u32) });
                }
            }
        }
        None
        ...
        let mut indent: usize = 0; let w: usize = 4; let mut prev = false;
        for line in source.lines() {
            let t = line.trim();
            if t.is_empty() { if !out.is_empty() { out.push('\n'); } prev = true; continue; }
            let close = t.chars().filter(|&c| c == '}' || c == ']' || c == ')').count();
            let open = t.chars().filter(|&c| c == '{' || c == '[' || c == '(').count();
            let net = open as isize - close as isize;
            if close > 0 && close >= open { indent = indent.saturating_sub((close - open) * w); }
            if t.starts_with("in ") || t == "in" { indent = indent.saturating_sub(w); }
            if !prev && !out.is_empty() { out.push('\n'); }
            for _ in 0..indent/w { out.push_str("    "); }
            out.push_str(t);
            if net > 0 { indent += net as usize * w; }
            if (t.ends_with("in") || t.ends_with("then")) && !t.contains('{') { indent += w; }
            prev = false;
        }
        if !out.ends_with('\n') { out.push('\n'); }
        out
        ...
        let mut tokens = Vec::new(); let mut pl = 0u32; let mut pc = 0u32;
        let mut line = 0u32; let mut col = 0u32;
        let bytes = source.as_bytes(); let mut i = 0;
        while i < bytes.len() {
            let ch = bytes[i] as char;
            if ch == '\n' { line += 1; col = 0; i += 1; continue; }
            if ch.is_whitespace() { col += 1; i += 1; continue; }
            if ch == '-' && i+1 < bytes.len() && bytes[i+1] == b'-' {
                let end = source[i..].find('\n').unwrap_or(source.len()-i);
                tokens.push(SemanticToken { delta_line: line-pl, delta_start: if line==pl{col-pc}else{col}, length: end as u32, token_type: 8, token_modifiers_bitset: 0 });
                pl = line; pc = col; col += end as u32; i += end; continue;
            }
            if ch == '"' {
                if let Some(end) = source[i+1..].find('"') {
                    let len = (end+2) as u32;
                    tokens.push(SemanticToken { delta_line: line-pl, delta_start: if line==pl{col-pc}else{col}, length: len, token_type: 5, token_modifiers_bitset: 0 });
                    pl = line; pc = col; col += len; i += end+2; continue;
                }
            }
            if ch.is_ascii_alphabetic() || ch == '_' {
                let start = i; while i < bytes.len() && (bytes[i].is_ascii_alphanumeric() || bytes[i] == b'_') { i += 1; }
                let word = &source[start..i]; let len = (i-start) as u32;
                let kw = ["let","fn","fun","actor","agent","workflow","if","else","match","case","for","in","spawn","send","ask","perform","handle","emit","return","break","unit","nil","true","false","iso","trn","ref","val","box","tag","type","effect","module","import","extern","self","and","or","not"];
                let tt: u32 = if kw.contains(&word) { 0 } else { 2 };
                tokens.push(SemanticToken { delta_line: line-pl, delta_start: if line==pl{col-pc}else{col}, length: len, token_type: tt, token_modifiers_bitset: 0 });
                pl = line; pc = col; col += len; continue;
            }
            if ch.is_ascii_digit() {
                let start = i; while i < bytes.len() && bytes[i].is_ascii_digit() { i += 1; }
                if i < bytes.len() && bytes[i] == b'.' { i += 1; while i < bytes.len() && bytes[i].is_ascii_digit() { i += 1; } }
                let len = (i-start) as u32;
                tokens.push(SemanticToken { delta_line: line-pl, delta_start: if line==pl{col-pc}else{col}, length: len, token_type: 6, token_modifiers_bitset: 0 });
                pl = line; pc = col; col += len; continue;
            }
            if "=+-*/<>!|&^%.:;{},()[]".contains(ch) {
                tokens.push(SemanticToken { delta_line: line-pl, delta_start: if line==pl{col-pc}else{col}, length: 1, token_type: 7, token_modifiers_bitset: 0 });
                pl = line; pc = col; col += 1; i += 1; continue;
            }
            col += 1; i += 1;
        }
        tokens
        ...
        for (li, line) in source.lines().enumerate() {
            let t = line.trim(); let ln = li as u32;
            if let Some(pos) = t.find("let ") {
                if !t[pos..].contains(':') {
                    let after = pos + 4; let rest = &t[after..];
                    if let Some(end) = rest.find(|c: char| c == ' ' || c == '=') {
                        let vname = &rest[..end]; let eq = t.find('=').unwrap_or(t.len());
                        let rhs = t[eq+1..].trim();
                        let ty = if rhs.parse::<i64>().is_ok() {"Int"} else if rhs.starts_with('"') {"String"} else if rhs == "true" || rhs == "false" {"Bool"} else {"a"};
                        let edit = TextEdit { range: Range { start: Position::new(ln, (pos+eq) as u32), end: Position::new(ln, (pos+eq) as u32) }, new_text: format!(" : {}", ty) };
                        let mut changes = std::collections::HashMap::new();
                        changes.insert(Url::parse("file:///current.nu").unwrap(), vec![edit]);
                        actions.push(CodeActionOrCommand::CodeAction(CodeAction {
                            title: format!("Add type annotation ': {}' for '{}'", ty, vname),
                            kind: Some(CodeActionKind::QUICKFIX), diagnostics: None,
                            edit: Some(WorkspaceEdit { changes: Some(changes), ..WorkspaceEdit::default() }),
                            command: None, is_preferred: Some(false), disabled: None, data: None }));
                    }
                }
            }
        }
