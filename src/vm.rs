        ...
        if effect_name == "Web" {
            match op_name {
                Some("html") => {
                    ...
                    out.push('<');
                    out.push_str(&tag);
                    if let Some(ptr) = regs.get(1).and_then(|v| v.as_ptr()) {
                        ...
                        if len > 0 {
                            ...
                            for attr in slots {
                                if let Some(attr_ptr) = attr.as_ptr() {
                                    let attr_len = self.array_len(attr_ptr).unwrap_or(0);
                                    if attr_len >= 2 {
                                        let attr_slots = unsafe {
                                            std::slice::from_raw_parts(
                                                attr_ptr as *const Value,
                                                attr_len,
                                            )
                                        };
                                        let name = resolve_value_string(constants, attr_slots[0]);
                                        let value = resolve_value_string(constants, attr_slots[1]);
                                        out.push(' ');
                                        out.push_str(&name);
                                        out.push_str("=\"");
                                        out.push_str(&value);
                    ...
                    out.push('>');
                    if let Some(ptr) = regs.get(2).and_then(|v| v.as_ptr()) {
                        let len = self.array_len(ptr).unwrap_or(0);
                        if len > 0 {
                            let slots = unsafe {
                                std::slice::from_raw_parts(ptr as *const Value, len)
                            ...
                            for child in slots {
                                out.push_str(&resolve_value_string(constants, *child));
                            }
                        }
                    }
                    out.push_str("</");
                ...
                Some("route") => return Some(Value::unit()),
                Some("text") => {
                    let s = regs
                        .first()
                        .map(|v| resolve_value_string(constants, *v))
                ...
                Some("read_body") | Some("param") | Some("header") => {
                    return Some(Value::nil())
                }
                _ => return None,
            }
...
impl VM {
    ...
    fn step_call(
        ...
        new_frame.regs[..argc as usize]
            .copy_from_slice(&self.frames[frame_idx].regs[..argc as usize]);
