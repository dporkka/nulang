...
pub fn generate_project_docs(root: &Path) -> NuResult<String> {
    ...
    for path in files {
        let source = fs::read_to_string(&path).map_err(|e| NuError::RuntimeError {
            msg: format!("docgen: cannot read {}: {}", path.display(), e),
            span: Span::default(),
        })?;
        let name = path
            .file_stem()
...
pub fn write_project_docs(root: &Path) -> NuResult<PathBuf> {
    ...
    fs::create_dir_all(&docs_dir).map_err(|e| NuError::RuntimeError {
        msg: format!("docgen: cannot create {}: {}", docs_dir.display(), e),
        span: Span::default(),
    })?;
    let out_path = docs_dir.join("api.md");
    fs::write(&out_path, markdown).map_err(|e| NuError::RuntimeError {
        msg: format!("docgen: cannot write {}: {}", out_path.display(), e),
        span: Span::default(),
    })?;
    Ok(out_path)
}

/// Scan `root/src/` for `.nula` files and write the combined API reference
/// to `root/docs/api.md`, returning the output path.
///
/// This is the package-mode variant: it limits the scan to the `src/`
/// directory and changes the attribution line to `nulang nula doc`.
pub fn write_package_docs(root: &Path) -> NuResult<PathBuf> {
    let src_dir = root.join("src");
    let markdown = generate_package_docs(&src_dir)?;
    let docs_dir = root.join("docs");
    fs::create_dir_all(&docs_dir).map_err(|e| NuError::RuntimeError {
        msg: format!("docgen: cannot create {}: {}", docs_dir.display(), e),
        span: Span::default(),
...
/// Generate the combined API reference for `.nula` files under `src_dir`,
/// with an attribution line for `nulang nula doc`.
fn generate_package_docs(src_dir: &Path) -> NuResult<String> {
    let mut files = Vec::new();
    collect_source_files(src_dir, &mut files)?;
    files.sort();

    let mut out = String::from(
        "# API Reference\n\n_Generated by `nulang nula doc`. Do not edit by hand._\n\n",
    );
    for path in files {
        let source = fs::read_to_string(&path).map_err(|e| NuError::RuntimeError {
            msg: format!("docgen: cannot read {}: {}", path.display(), e),
...
fn collect_source_files(dir: &Path, files: &mut Vec<PathBuf>) -> NuResult<()> {
    let entries = fs::read_dir(dir).map_err(|e| NuError::RuntimeError {
        msg: format!("docgen: cannot list {}: {}", dir.display(), e),
        span: Span::default(),
    })?;
    for entry in entries {
        let entry = entry.map_err(|e| NuError::RuntimeError {
            msg: format!("docgen: cannot list {}: {}", dir.display(), e),
            span: Span::default(),
        })?;
        let path = entry.path();
