...
pub fn generate_project_docs(root: &Path) -> NuResult<String> {
    ...
    for path in files {
        let source = fs::read_to_string(&path).map_err(|e| {
            NuError::RuntimeError { msg: format!("docgen: cannot read {}: {}", path.display(), e), span: Span::default() }
        })?;
        let name = path
            .file_stem()
            .and_then(|s| s.to_str())
...
pub fn write_project_docs(root: &Path) -> NuResult<PathBuf> {
    ...
    fs::create_dir_all(&docs_dir).map_err(|e| {
        NuError::RuntimeError { msg: format!(
...
fn collect_source_files(dir: &Path, files: &mut Vec<PathBuf>) -> NuResult<()> {
    let entries = fs::read_dir(dir).map_err(|e| {
        NuError::RuntimeError { msg: format!("docgen: cannot list {}: {}", dir.display(), e), span: Span::default() }
    })?;
