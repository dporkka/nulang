...
- **Instruction** (`src/bytecode.rs`): 32-bit fixed-width `{opcode:u8, op1:u8, op2:u8, op3:u8}`; helpers `new0/1/2/3`, `imm16()`, `simm16()`, `offset16()`. 137 opcodes across 17 category ranges (Special, Stack & Locals, Int/Float Arithmetic, Comparison & Logic, Control Flow, Closures, Memory & Objects, Actor & Concurrency, Effects, Python Interop, FFI, Supervisor, Debate, Distribution, String & IO, Debug & Meta).
...
- `src/bytecode.rs` — `OpCode` (137), `Instruction` (32-bit), `Constant`, `CodeModule`.
